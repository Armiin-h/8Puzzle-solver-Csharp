...
namespace _8PuzzleGame
{
    public class State : IComparable<State>
    {
        ...
        public int mDistance()
        {
            ...
            for (int i = 0; i < matrix.GetLength(0); i++)
            {
                for (int j = 0; j < matrix.GetLength(1); j++)
                {
                    ...
                    switch (num)
                    {
                        case 1: this.Distance += Math.Abs(0 - i) + Math.Abs(1 - j); break;
                        case 2: this.Distance += Math.Abs(0 - i) + Math.Abs(2 - j); break;
                        case 3: this.Distance += Math.Abs(1 - i) + Math.Abs(0 - j); break;
                        case 4: this.Distance += Math.Abs(1 - i) + Math.Abs(1 - j); break;
                        case 5: this.Distance += Math.Abs(1 - i) + Math.Abs(2 - j); break;
                        case 6: this.Distance += Math.Abs(2 - i) + Math.Abs(0 - j); break;
                        case 7: this.Distance += Math.Abs(2 - i) + Math.Abs(1 - j); break;
                        case 8: this.Distance += Math.Abs(2 - i) + Math.Abs(2 - j); break;
        ...
        {
            // Heuristic value
            var thisValue = this.mDistance();
            var otherValue = other.mDistance();
